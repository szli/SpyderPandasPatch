...
def get_size(item):
    ...
    if isinstance(item, (list, tuple, dict)):
        return len(item)
    elif isinstance(item, (ndarray, MaskedArray, Series, DataFrame)):
        return item.shape
...
def is_known_type(item):
    ...
    return isinstance(item, MaskedArray) or isinstance(item, (Series, DataFrame)) or get_type_string(item) is not None

def get_human_readable_type(item):
    """Return human-readable type string of an item"""

    if isinstance(item, DataFrame):
        if unique(item.dtypes).size == 1:
                return unique(item.dtypes)[0].name + ' DataFrame'
        else:
                return "Hetero-DataFrame"
    if isinstance(item, Series):
        return item.dtype.name + ' Series'
