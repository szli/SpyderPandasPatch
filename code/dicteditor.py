...
class DictDelegate(QItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        if isinstance(value, (list, tuple, dict)) and not self.inplace:
            ...
            return None
        #---editor = ArrayEditor
        elif isinstance(value, (ndarray, MaskedArray)) or isinstance(value, (Series, DataFrame))\
             and ndarray is not FakeObject and not self.inplace:

            xlabels, ylabels = None, None
            if (isinstance(value, (Series, DataFrame))):  #Not sure why Series worked without this...
                ylabels = [str(x) for x in value.index]
                
                if (isinstance(value, DataFrame)):
                    xlabels = [str(x) for x in value.columns]
            ...
            if not editor.setup_and_check(value, title=key, readonly=readonly, xlabels=xlabels, ylabels=ylabels):
                return
            self.create_dialog(editor, dict(model=index.model(), editor=editor,
                                            key=key, readonly=readonly))
            return None
